...
def add_user(name: str, email: EmailStr, token: str, engine):
    add_single(User(name=name, email=email, token=token), engine)


def lookup_user(id: UUID4, engine) -> User | None:
    with Session(engine) as session:
        return session.exec(select(User).where(User.id == id)).first()


def lookup_user_by_email(email: EmailStr, engine) -> User | None:
    with Session(engine) as session:
